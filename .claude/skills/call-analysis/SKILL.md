...
### 🎚 Команды управления видео (договорённость с пользователем)
- **«прочитай видео»** → только **транскрипт** (быстро, дёшево). Источник по типу ссылки — шаг 0.
- **«просмотри видео»** → транскрипт **+ кадры с OCR** (вижу, что на экране). Кадры — точечно по
  ключевым моментам (или scene-detect `timestamps='[]'` для чистых скрин-туторов). Это медленнее.
- **Команда не указана → по умолчанию «прочитай»** (транскрипт). Если по транскрипту видно, что суть
  на экране (скрин-демо, «вот тут / как видите / эта галочка») — предложи «просмотреть».
...
- **Скриншот / изображение** (PNG/JPG: переписка, заметки со встречи, доска, протокол) →
  читай напрямую инструментом **Read** (нативно, без инфраструктуры). Вытащи из картинки текст,
...
**4.0. Сначала защита от дублей.** Один и тот же звонок могут прислать повторно. Перед созданием
задач проверь, нет ли уже задач с этим же **«Источник встречи»** (одинаковое название встречи + дата):

```
notion-search (или query data source collection://62fe1adb-5e94-4e00-ae0e-35c6ee802dac)
по значению «Источник встречи» = "<название> — <дата>"
```

- **Совпадений нет** → создавай задачи как обычно.
- **Совпадения есть** → НЕ создавай дубли. Сверь список: новых задач (которых ещё нет по тексту
  «Задача») — добавь; уже существующие — пропусти (при изменении срока/статуса — обнови существующую
  страницу, а не плоди копию). В итоге скажи пользователю: «эта встреча уже разбиралась, добавил N новых,
  пропустил M существующих».

Поэтому всегда пиши **«Источник встречи»** в едином формате `Название — ДД.ММ.ГГГГ` — это ключ дедупликации.
