...
## Email-інтеграція — технічні деталі (ChatGPT Deep Research, червень 2026)

- **Синхронізація IMAP:** Email-повідомлення синхронізуються через polling кожні **5 хвилин**. Не real-time.
- **Email Tracking (відкриття/кліки):** доступний лише починаючи з тарифу **Advanced+**.
- **Підтримувані клієнти:** Gmail (OAuth), Outlook 365 (OAuth), будь-який IMAP/SMTP.
- **Двонаправлений sync:** вхідні та вихідні листи менеджерів автоматично логуються до картки угоди/контакту.
- **Шаблони листів:** Налаштування → Автоматизація → Шаблони листів (підтримка змінних: ім'я клієнта, сума, кастомні поля).

## Міграція — рекомендований порядок об'єктів

При міграції через API рекомендується дотримуватись такого порядку:
1. **Компанії** (Companies)
2. **Контакти** (Contacts)
3. **Ліди / Угоди** (Leads)
4. **Задачі** (Tasks)
5. **Примітки** (Notes)

Це запобігає проблемам з прив'язкою (лід не може посилатись на контакт, який ще не створено).
