...
- **Developer Portal:** https://uspace.dev — хаб для розробників (SDK, темплейти, документація)
...
| SDK | Стан |
|-----|------|
| **Go** | Повний, production-ready (основна мова бекенду Uspacy) |
| **TypeScript/JavaScript** | Повний |
| **PHP** | Частковий |

### Темплейти застосунків
- Laravel (PHP)
- JavaScript (Node.js/Express)
- Приклад UI-віджету для правої панелі

### Мікрофронтенди
- Дозволяють вбудовувати власні UI-компоненти в інтерфейс Uspacy
- Точки вбудови: ліве бокове меню, верхнє меню, картки сутностей, права колонка
- Стратегічне рішення — закладено з першого дня (CTO Кирило Мельничук, вебінар #11, 12.2025)

### Технічна архітектура (з вебінару #11, 02.12.2025)
- **Архітектура:** мікросервісна, multi-tenant з фізично розділеними БД на клієнта
- **Хостинг:** Google Cloud + AWS (hybrid)
- **Frontend:** мікрофронтенди
- **MongoDB** — чат
- **DynamoDB** — реакції / сповіщення
- **Elasticsearch** — пошук і фільтри
- **PostgreSQL** — реляційні дані (CRM, Tasks)
- **AI моделі:** Google, Anthropic (Claude), OpenAI — вибір залежно від задачі
- **API-first:** внутрішнє і зовнішнє API однакові — всі UI-дії доступні через API

### Вайб-кодинг з Uspacy API (вебінар #15, 26.05.2026)
- Claude Code, Cursor, Windsurf — рекомендовані інструменти
- Кейс: інтегратор Rubikon написав MVP застосунку «Погодження» за 3 дні без розробників
- Кейс: директор продукту написав партнерський кабінет без розробника (3 дні MVP, 2 тижні до релізу)
- Рекомендований підхід: описати бізнес-задачу → попросити план → ітеративна розробка
- Для AI-агентів: надати документацію API як контекст → якість значно краще
- Make/n8n кращі за пряме використання Claude для AI-автоматизацій (дешевше per token)
